import { CommandInteraction, Message, GuildMember, MessageEmbed, ButtonInteraction, MessageActionRow, MessageButton } from "discord.js";
...
export class Love {
    ...
    async minfo(
        ...
        try {
            ...
            if (user?.partner == "") {
                ...
                var partner;
                if (partnerFetch == undefined) partner = "«Не найдено»"
                else partner = `${partnerFetch.user.username}#${partnerFetch.user.discriminator}`
    ...
    async marry(
        @SimpleCommandOption("muser", { type: SimpleCommandOptionType.User })
        muser: GuildMember | undefined,
        command: SimpleCommandMessage
    ) {
        // if..
        try {
    ...
    async divorce(
        ...
        try {
            ...
            if (user?.partner == "") {
                ...
                try { await muser?.roles.remove("928368525045825567"); } catch (e) {}
                try { await command.message.member?.roles.remove("928368525045825567"); } catch (e) {}
